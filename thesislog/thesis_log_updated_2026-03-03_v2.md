...
## Update (2026-03-03) – 50 cm Messung + ML-Dataset Export

### 50 cm Ergebnisse (Near-Field, r=0.50 m, Multi-Window + Mode, bandpass 500–6000 Hz)
Messwinkel: 0°, 90°, 140°, 180°.
- 0° → 0°
- 90° → 90°
- 140° → 135° (≈ −5°)
- 180° → 180°
Interpretation: sehr stabile Schätzungen (hoher Anteil nutzbarer Fenster). Der 140°-Fall zeigt einzelne Ausreißer-Fenster (z. B. Richtung 270°), aber der dominante Cluster bleibt korrekt → Robustheit durch Multi-Window+Mode.

### ML-Basis (Export)
- Fenster-basierter Export in `ml_windows_calib.csv`:
  - pro Fenster: (distance_m, angle_gt_deg, t_center_s, theta_raw_deg, theta_calib_deg, confidence, top1/top2, used_flag, calib_file)
- Das ist eine sinnvolle **Feature/Label-Basis**, um später z. B. einen Klassifikator/Regressor zu trainieren oder Confidence-Rekalibrierung zu lernen.
- Hinweis: `used_flag` ist in der CSV als String gespeichert ("0"/"1") → beim Auswerten in Python in int casten.

### Offene Punkte für ML
- Mehr Datenpunkte für 1.81 m (v. a. Zwischenwinkel wie 60°) – dort treten noch starke Raum/Mehrwege-Effekte auf.
- Für ML: pro Winkel/Distanz mehrere Takes (≥3) aufnehmen, damit Verteilungen (Streuung, Outlier-Rate) sichtbar werden.

### ToDo (nächster Block)
- Distanzserie erweitern (z. B. 0.80 m, 1.20 m, 1.50 m) mit 0/90/140/180.
- Plots: MAE vs. Distanz, Error vs. Winkel, used_ratio/confidence vs. Fehler.
